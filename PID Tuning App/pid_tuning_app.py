#!/usr/bin/env python3
"""
PID Tuning Application - Version 1.0.0
...
class PIDTuningApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PID Tuning Application - Version 1.0.0")
        ...
        self.root.bind('<Control-s>', lambda e: self.start_control())
        self.root.bind('<Control-x>', lambda e: self.stop_control())
        self.root.bind('<Control-d>', lambda e: self.debug_var.set(""))  # Clear debug
...
Debug messages from the Arduino appear in the status bar:
- Shows when start/stop commands are received
- Displays control state changes
- Messages auto-clear after 3 seconds
- Press Ctrl+D to manually clear debug messages

Use this to verify:
- Commands are being received
- Control is actually starting/stopping
- Output is being set to 0 when stopped
...
"""
    ...
    def setup_status_bar(self, parent):
        ...
        ttk.Label(left_frame, textvariable=self.status_var).pack(side=tk.LEFT)
        
        # Debug messages
        ttk.Label(left_frame, text=" | Debug: ").pack(side=tk.LEFT, padx=(10, 0))
        self.debug_var = tk.StringVar(value="")
        self.debug_label = ttk.Label(left_frame, textvariable=self.debug_var, foreground="lightblue")
    ...
    def connect_serial(self):
        ...
        try:
            ...
            # Enable control buttons with correct initial states
            self.start_btn.config(state='normal')
            self.stop_btn.config(state='disabled')  # Initially stopped
                ...
                if self.serial_port.in_waiting:
                    ...
                    while '\n' in buffer:
                        ...
                        if line:
                            try:
                                msg = json.loads(line)
                                # Only add valid JSON messages with 'type' field
                                if isinstance(msg, dict) and 'type' in msg:
                                    self.data_queue.put(msg)
                            except json.JSONDecodeError as e:
                                print(f"JSON decode error: {e} - Line: {line}")
                            except Exception as e:
                                print(f"Error processing message: {e} - Line: {line}")
        ...
        try:
            while True:
                ...
                if msg['type'] == 'data':
                    ...
                    debug_msg = msg.get('debug', '')
                    self.debug_var.set(debug_msg)
                    # Clear debug message after 3 seconds
                    self.root.after(3000, lambda: self.debug_var.set(""))
    ...
    def start_control(self):
        ...
        self.root.after(100, lambda: self.send_command("get_status"))
